...
interface TaxInformationFormProps {
  onSubmit: (data: TaxFormData) => Promise<TaxCalculationResult>;
...
export const TaxInformationForm: React.FC<TaxInformationFormProps> = ({ 
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<TaxFormData>({
    ...
    defaultValues: {
      ...
      user_age: initialData?.user_age || 25,
      is_senior_citizen: initialData?.is_senior_citizen || false,
      investments: {
        '80C_investments': initialData?.investments?.['80C_investments'] || 0,
        nps_contribution: initialData?.investments?.nps_contribution || 0
      },
      health_insurance_premium: initialData?.health_insurance_premium || 0,
      parents_health_insurance_premium: initialData?.parents_health_insurance_premium || 0,
      medical_expenses: initialData?.medical_expenses || 0,
      parents_age: initialData?.parents_age || 0,
      housing_loan_interest: initialData?.housing_loan_interest || 0,
      property_status: initialData?.property_status || 'self_occupied',
      donation_amount: initialData?.donation_amount || 0,
      education_loan_interest: initialData?.education_loan_interest || 0,
      disability_details: {
        is_disabled: initialData?.disability_details?.is_disabled || false,
        type: initialData?.disability_details?.type
      },
      other_income: {
        interest_from_savings: initialData?.other_income?.interest_from_savings || 0,
        fixed_deposit_interest: initialData?.other_income?.fixed_deposit_interest || 0
      }
    }
  });
